    ...
    <div id="admin-comments">
        ...
        <main id="admin-comments-list">
            <transition-group enter-active-class="part-enter-2" leave-active-class="part-leave-2">
                <p ref="posts" :key="data+1" v-for="(item,data) in comments" >{{comments[data].time}} , {{comments[data].name}} : {{ comments[data].content }}<button @click="DelComment(`${comments[data].name}`,`${comments[data].content}`,`${comments[data].time}`)"><i class="fa fa-trash-o fa-lg"></i> 删除</button></p>
            </transition-group>
        </main>
    </div>
</template>
